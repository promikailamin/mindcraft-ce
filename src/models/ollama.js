...
export class Ollama {
    ...
    constructor(model_name, url, params) {
        ...
        this.params = params;
        this.url = url || process.env.OLLAMA_URL || 'http://192.168.2.104:11434';
    ...
    async embed(text) {
        ...
        let res = null;
        try {
            res = await this.send(this.embedding_endpoint, body);
        } catch (err) {
            if (String(err.message).includes('404')) {
                res = await this.send('/api/embeddings', body);
            } else {
                throw err;
            }
        }
    ...
    async send(endpoint, body) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                throw new Error(`Ollama Status: ${res.status} ${res.statusText}`);
            }
        } catch (err) {
            console.error('Failed to send Ollama request to', this.url, endpoint);
            console.error(err);
            throw err;
        }
        return data;
